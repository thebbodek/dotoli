...
- **`peerDependencies`는 반대로 `external`에 넣지 않습니다.** `peerDepsExternal()`이 잡고 그쪽은 **서브패스까지 처리**하므로, `next`를 peerDep으로 선언하면 `next/link`도 external로 나갑니다 (DOTOLI-303 실측 — `external` 항목 없이 빌드해도 인라인되지 않습니다). `react` · `react-dom` · `tailwindcss`가 목록에 없는 것과 같은 이유입니다.

  **`dependencies`로 넣는 순간 이 보호가 사라집니다.** `apps/internal-ui`가 `next`를 dependencies에 두고 `external`에도 안 넣어, `next/link` 소스가 dist에 통째로 인라인돼 있습니다(`index` 청크 352KB, dist가 `next`를 import 하는 곳 0건). 번들된 사본은 자기 사본의 라우터 context를 보므로 앱의 라우터와 인스턴스가 갈립니다.
