...
- [ ] (번호 미정) biz-ui LinkNavigationListItem 추가 — 라우트 이동과 바텀시트 트리거로 둘 다 쓰여 전환이 아니라 링크판을 따로 세웁니다. 버튼판은 시트 트리거로 유지 — [components/navigation-list-item.md](./components/navigation-list-item.md) 「결정」
