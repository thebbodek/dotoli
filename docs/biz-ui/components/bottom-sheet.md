...
- **헤더는 고정, 바디만 스크롤합니다.** 시트가 `flex-v-stack` + 높이 상한(아래)이고 바디가 `flex-1 min-h-0 scroll-y`입니다. `Overlay`가 `fixed inset-0` + `items-end`라 시트가 콘텐츠 높이로 자라는데, **막지 않으면 긴 바디에서 헤더가 화면 위로 밀려 나가 닫기 버튼에 닿을 수 없습니다.** 시각을 더한 것이 아니라 넘침을 막은 것이라 「Figma에 없는 시각은 만들지 않는다」에 걸리지 않습니다.

- **긴 시트는 위에 `50px`을 남깁니다 — `max-h-[calc(100%-50px)]`.** 처음엔 `max-h-full`이라 꼭대기까지 찼습니다. Figma 심볼은 79를 남기는데(619 중 540) 그 값을 일반화할 근거가 없어 「디자인 확인 필요」로 올렸고, **디자이너가 「50px 또는 6% 중 적당한 쪽」으로 답을 줘 고정 50px을 골랐습니다** (DOTOLI-275).

  | | 실기기 높이 범위에서 남는 띠 |
  | --- | --- |
  | **고정 50px** | 어디서나 **50** |
  | 비율 6% | SE 667 → 40 · 14~16 852 → 51 · Pro Max 956 → 57 |

  **고정을 고른 이유는 세 가지입니다.** ① 이 DS에는 비율 기반 크기가 **한 곳도 없습니다** — 전부 고정 px이고, 같은 컴포넌트의 `max-w-[480px]`이 바로 옆의 같은 성격 결정입니다. ② 띠는 「뒤에 층이 있다」는 시각 신호라 **화면 높이에 비례할 이유가 없습니다.** ③ 6%는 40~57로 흔들려 디자이너가 확인할 수치가 기기마다 달라집니다.

  390 × 844(뷰포트 892)에서 재보니 시트가 **842 = 892 − 50**이고 위에 딤 띠가 그대로 남습니다.
...
- **`max-w-[480px] mx-auto`로 넓은 화면에서 가운데에 세웁니다.** biz-ui는 모바일 단일 타깃이지만 **바텀시트는 PC에서 열릴 수 있다**는 요구가 들어왔습니다(DOTOLI-274). 상한이 없으면 시트가 화면 폭을 그대로 먹고, `Overlay`의 바텀시트 배치가 `flex-h-stack items-end`(가로 정렬은 기본값 `flex-start`)라 **좁히기만 하면 왼쪽에 붙습니다.**
  ...
  **480인 이유** — 현행 최대 폭 단말(iPhone Pro Max 430)보다 커야 **실기기에서는 지금처럼 꽉 찹니다.** Figma 문서 프레임 380을 그대로 상한으로 쓰면 큰 폰에서 좌우가 비어 오히려 회귀입니다. 375 · 980에서 실측해 각각 「폭 꽉 참」 · 「480 가운데」를 확인했습니다. `--container-*` 스케일이 아직 없어 임의값이지만 **480으로 확정했습니다**(DOTOLI-275 확인).

  **이 값이 시트 안쪽 배치에 실제로 영향을 줍니다.** [`DateBottomSheet`](./calendar.md#datebottomsheet)의 연도 칩이 `flex-wrap`이었을 때 380에서는 한 줄 4개인데 480에서 5개가 됐습니다 — 그래서 그쪽을 4열 그리드로 고정했습니다. **폭에 의존하는 배치는 380이 아니라 480에서도 확인해야 합니다.**
