...
### 웨이트는 `fill`입니다 — Figma와 일치합니다

`BOTTOM_TAB_ICON_WEIGHT = ICON_WEIGHTS.FILL`. `Icon` 기본값(`bold`)이 아니라 명시합니다.

크기 축(26 · 26 · 28)과 색은 웨이트와 무관합니다.

**좌표 대조만으로는 판정할 수 없습니다 — 여기서 한 번 틀렸습니다.** Figma export의 첫 점 `M20.28 5.09139`를 26px 박스 · 글리프 원점 (24, 16) · 배율 26/256으로 되돌리면 `223.68 · 66.15`인데, **`regular`와 `fill`의 원본 path가 이 첫 점을 공유합니다.** 바깥 실루엣이 같은 글리프라 시작점도 같습니다.

| 웨이트    | 원본 첫 점     | 26px 환산        | Figma export           |
| --------- | -------------- | ---------------- | ---------------------- |
| `regular` | `223.68,66.15` | 20.28 · 5.0934   | 일치 — **가르지 못함** |
| `fill`    | `223.68,66.15` | 20.28 · 5.0934   | 일치 — **가르지 못함** |
| `bold`    | `225.6,62.64`  | 20.4750 · 4.7369 | 불일치                 |

글리프 폭도 같습니다 — 세 아이콘 모두 정확히 208/256 단위(좌우 24 인셋)인데 이건 `regular`·`fill` 공통이고 `bold`만 밀립니다. **즉 첫 점과 바운딩은 `bold`를 걸러낼 뿐입니다.**

**가르는 것은 서브패스 개수입니다.** `fill`은 속을 채워 내부 구획선이 사라지므로 `regular`보다 서브패스가 하나 적습니다.

| 소스                    | `d` 안의 `M`/`m` |
| ----------------------- | ---------------- |
| `regular/package.svg`   | 5                |
| `fill/package-fill.svg` | **4**            |
| Figma export            | **4**            |

**활성 · 비활성은 웨이트가 같습니다.** 같은 아이콘의 두 상태 SVG를 받아 비교하면 `fill` 속성 한 곳만 다르고 path는 바이트 단위로 동일합니다. 화면에서 활성 아이콘이 굵어 보이는 것은 색(`base/black`) 때문입니다 — 「비활성은 외곽선, 활성만 채움」은 흔한 바텀탭 패턴이지만 Figma에 그 축이 없습니다.

> checkbox.md의 `check` 판정(글리프 바운딩 `getBBox()` 비교)은 `regular` ↔ `bold`를 가르는 것이라 이 함정에 걸리지 않습니다. **`regular` ↔ `fill`을 가를 때만 바운딩·시작점이 무력해집니다.**
