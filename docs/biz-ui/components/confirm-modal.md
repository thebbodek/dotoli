...
- **COM-011(버튼 라벨 규칙)을 코드로 강제하지 않습니다.** 섹션 안 정책(`524:10`)이 「좌측 — 파괴적 액션 / 우측 — 유지 액션」을 요구하는데, **DS는 어느 쪽이 파괴적인지 알 수 없습니다.** 라벨과 핸들러를 소비자가 넘기고 스타일은 자리로 고정(좌 tonal gray · 우 filled primary)합니다. 정책 준수는 소비 앱 몫이고, 아래에 어긋난 지점을 남겼습니다.
  ...
  **`confirm` · `cancel`은 라벨이 아니라 역할 이름입니다.** COM-011이 자리의 역할을 고정하므로 `cancel`은 **하던 것을 버리는 쪽**, `confirm`은 **지키는 쪽**입니다 — 아래 예시의 `cancel={{ label: '삭제' }}`가 어긋난 것으로 보이지만 정책 그대로입니다. **정책이 역할을 고정해 주기 때문에 이 이름이 성립합니다.** 같은 이름을 [`BottomActionBar`](./bottom-action-bar.md)가 쓰다가 DOTOLI-277에서 `action` · `subAction`으로 개명한 것은 **거기에 대응하는 정책이 없어** 왼쪽이 취소·이전·링크로 갈리기 때문입니다.
