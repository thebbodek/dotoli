...
- **월 ↔ 연도 연결은 DS가 아니라 소비 앱이 배선합니다.** [`CalendarBottomSheet`](#calendarbottomsheet)의 「연도 시트 연결 — DOTOLI-275에서 확인」 항목이 여기로 닫힙니다. **연결에 필요한 것이 전부 공개 API로 이미 있습니다** — `dateSelectOption.onYearClick`이 뷰를 바꾸고, `type` · `options` · `value`가 그 뷰를 그립니다(`title`은 `type`에서 파생됩니다). `Linked` 스토리가 그 배선을 그대로 보여 줍니다.

  **연결 동작은 확인받았고 계약으로 둡니다** (DOTOLI-275). 코드로 강제하지 않으므로 소비 앱이 지킵니다.

  | 트리거 | 동작 |
  | --- | --- |
  | 월 시트의 `26년▼` 탭 | **연도 뷰로 교체** |
  | 연도 뷰에서 연도 선택 후 확정 버튼 | 연도를 적용하고 **월 뷰로 복귀** |
  | 연도 뷰의 닫기 · 배경 탭 | **월 뷰로 복귀** (시트는 닫지 않음) |

  **시트는 하나만 마운트하고 내용만 갈아 끼웁니다** (DOTOLI-286). 시트를 둘 두고 `isOpen`을 번갈아 켜는 배선이 더 직관적으로 보이지만, [`Overlay`](./overlay.md)는 「닫힘 애니메이션은 없습니다」대로 **`isOpen=false`면 즉시 언마운트**합니다. 그래서 한쪽이 사라지고 다른 쪽이 **같은 커밋에서 새로 마운트**돼 `animate-bottom-sheet`(250ms 슬라이드업)과 딤의 `animate-fade-in`이 처음부터 다시 재생됩니다. 사용자 눈에는 **하나의 시트가 리셋되는 깜빡임**으로 보입니다 — 내용만 바뀌기를 기대하는 자리라 어긋납니다.

  `isOpen`을 켜 둔 채 props만 바꾸면 React가 `<dialog>`와 콘텐츠 래퍼를 **재사용**하므로 애니메이션이 다시 돌지 않습니다. Storybook 렌더로 월 → 연도 → 월 왕복 내내 두 노드가 동일하고 열린 `dialog`가 계속 1개인 것을 확인했습니다.

  **별개 오버레이를 위에 쌓는 경우(COM-008 이탈 모달 등)는 해당하지 않습니다** — 아래 시트가 마운트를 유지해 재생될 것이 없습니다. 딤이 두 겹으로 겹치므로 위쪽에 `isDimmed={false}`를 줍니다.
...
| 뷰 교체 시 높이 점프  | 월(12칩 3행) ↔ 연도(칩 수 가변)의 높이가 달라 교체 순간 시트가 즉시 늘거나 줄어듭니다. 재진입 깜빡임과 별개이고 모션 정의가 없어 그대로 뒀습니다 |
...
| `Linked` | **월 시트의 `26년▼` → 연도 뷰**. Figma에 연결 정의가 없어 구현으로 확인하는 자리 — 시트 하나를 유지한 채 내용만 교체해 **재진입 깜빡임이 없는 것**을 보는 자리이기도 하다 |
