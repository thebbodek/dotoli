...
### DOTOLI-297 · `title`을 `ReactNode`로 · `highlight` 제거

[`Toast`](./toast.md)와 **함께 나갔습니다** — 위 「강조부 위치」가 「Toast와 제약을 공유한다」로 묶어 둔 그대로입니다. 전환의 계기 · 근거 · 함정은 [toast.md](./toast.md) 「DOTOLI-297」에 있고, 여기에는 **이 컴포넌트에서만 갈리는 것**을 적습니다.

- **`highlight` + `title` 두 prop이 `title` 하나가 됐습니다.** 위 결정문의 `ReactNode` 거부 근거 둘 중 **②(biz-ui에 `ReactNode` prop 선례가 없음)는 순환논리였습니다** — 「안 해봤으니 안 한다」이고, [CLAUDE.md](../../../apps/biz-ui/CLAUDE.md) 「작성 후 검토」 2항은 정반대로 「internal-ui에 같은 성격 구현이 있는지 **먼저 찾아보고 없을 때 새로 설계**」를 지시합니다. internal-ui `Alert`가 `ReactNode`인데 그것을 보지 않고 닫은 것이라 규칙을 거꾸로 적용했습니다. **①(색 결정이 소비처로 샘)은 유효**해서, 색은 `[&_strong]:`로 DS가 계속 소유합니다.

  「나뉘지 않는 케이스」는 그대로 살아 있습니다 — `<strong>` 없이 문자열만 주면 `gray/800` 한 색으로 그립니다. **한 `Typography` 안에 두는 이유(강조부와 나머지가 같은 흐름에서 함께 줄바꿈)도 그대로**이고, 이제는 소비자가 넘긴 JSX가 그 흐름 안에 들어옵니다.

- **`[&_strong]:text-heading-4`로 굵기를 되돌려 박습니다.** 여기가 Toast와 갈리는 지점입니다. 타이틀이 `heading-4`(600)라 **맨몸 `<strong>`이 preflight의 `bolder`를 타고 900으로 튑니다** — 렌더에서 실측했습니다. Toast는 강조가 `text-body-bold`(700)를 명시적으로 얹으므로 이 문제가 드러나지 않습니다. 규칙은 [CLAUDE.md](../../../apps/biz-ui/CLAUDE.md) 「스타일 규칙」에 있습니다.
