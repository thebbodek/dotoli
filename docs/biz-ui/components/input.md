...
- **`height`에 하한 93px을 겁니다 (DOTOLI-301).** 소비 앱 플레이그라운드에서 발견했습니다 — 박스가 `flex-v-stack`이고 **라벨이 축소 대상이라 어디에도 보호 장치가 없습니다.** 40px을 주면 라벨까지 눌려 입력 영역이 0px이 되고 라벨과 값이 겹칩니다.

  **93은 「두 줄이 들어가는 최소」입니다.** 세 토큰에서 나옵니다 — `py-[10px]` 20 + `text-label` 20.3(14px × 1.45) + `text-body-lg-semibold` 26.1(18px × 1.45) × 2줄 = 92.5.

  한 줄만 보장하는 67px도 가능하지만 그러면 기능적으로 `InputField`(70px)와 같아집니다 — 같은 라벨 구조에 한 줄 입력이니까요. **TextArea라는 이름이 두 줄을 요구합니다.**

  **`min-height` CSS가 아니라 `Math.max`로 겁니다.** CSS로 걸면 소비자가 넘긴 `height`와 실제 렌더 높이가 말없이 달라집니다. `es-toolkit`의 `clamp`도 쓰지 않았습니다 — 2인자 형태가 상한으로 해석돼 정반대가 되고, 여기는 하한만 있습니다.

  **타이포 토큰이 바뀌면 이 값이 조용히 틀어집니다.** 유도식을 여기 남겨 두는 이유입니다.
