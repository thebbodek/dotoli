...
biz-ui는 `overlay-kit`에 의존하지 않습니다. **DOTOLI-265에서 「넣지 않는다」로 결정됐고, DOTOLI-270에서 확정됐습니다** — 아래 기준 1번(「biz-ui 컴포넌트가 자기 안에서 다른 오버레이를 여는가」)에 `ConfirmModal`도 [`BottomSheet`](./bottom-sheet.md)도 해당하지 않고(COM-008 이탈 모달을 소비 앱이 띄웁니다), 따라서 기준 2번도 성립하지 않습니다. **오버레이 실물 2종이 모두 「아니오」라 기준을 다시 볼 티켓이 남아 있지 않습니다.** 새 오버레이가 추가되면 그때 다시 봅니다.
