...
- **아이콘 박스를 `size-*`로 함께 고정했습니다.** Phosphor 글리프의 advance가 0.865em이라 `text-[16px]`만 주면 **박스가 13.84**가 되고 문구가 2px 왼쪽으로 밀립니다(닫기는 `text-[14px]` → 12.12). 맞추는 대상은 글리프가 아니라 **Figma 프레임에 해당하는 레이아웃 상자**라, 박스만 16 · 14로 잡고 글리프는 그 안에서 그려지게 둡니다. [`InfoBanner`](./info.md)가 같은 함정을 밟고 남긴 처리이고 `Divider` · `InfoItem`도 같습니다.

- **높이를 고정하지 않았습니다.** 36은 `py-[8px]` + `label-bold` 행높이(20.3)의 자연 높이입니다. 테두리가 없어 `MenuItem`처럼 stroke를 보정할 일도 없습니다.
...
- **버튼 둘에 `TOUCH_TARGET_STYLE`(6px)을 걸었습니다.** Figma 주석이 없는데도 넣은 **유일한 예외**입니다 — 시각 크기가 곧 히트 영역이면 보기 23.38×20.3, 닫기 14×14로 **둘 다 WCAG 2.5.8(24×24) 미달**이고, 특히 닫기는 10px이 모자라 모바일 WebView에서 그냥 안 눌립니다.

  **`TOUCH_TARGET_NARROW_STYLE`(4px)로는 부족합니다.** 닫기가 22×22에서 멈춰 여전히 미달입니다. 6px이면 보기 35.38×32.3 · 닫기 26×26으로 둘 다 넘깁니다.

  **대신 두 히트 영역이 정확히 맞닿습니다** — 버튼 사이 gap이 12라 `6 × 2 = 12`입니다. CLAUDE.md가 막는 것은 `확장 × 2 > 간격`인 **겹침**이고 여기는 경계를 공유할 뿐이라 엉뚱한 쪽이 눌리지는 않지만, 경계의 dead zone이 0이라는 것은 알고 고른 값입니다. 배너 안쪽으로는 위아래 2px · 오른쪽 7px이 남아 넘치지 않습니다.

  CLAUDE.md 「히트 영역 확장」대로 **`relative`를 버튼에 함께 겁니다.** 배너 루트가 꼬리 때문에 이미 `relative`라, 안 걸면 `::before`가 배너 전체를 덮어 배너 아무 데나 눌러도 버튼이 눌립니다. `cursor`도 같은 이유로 확장을 얹은 버튼에 둡니다.
