...
- **`next`가 필수 peerDependency입니다 (DOTOLI-303).** `LinkCtaButton` · `LinkIconButton`이 `next/link`를 직접 렌더하면서 **biz-ui가 처음으로 프레임워크에 묶였습니다.** **DOTOLI-304의 `BottomTabItem` · DOTOLI-306의 `LinkNavigationListItem` · DOTOLI-312의 `HeaderBarNotificationLink`도 여기 들어갑니다** — 의존을 재검토할 일이 생기면 영향 범위가 다섯입니다. 바로 위 항목의 독립성 원칙과 충돌하지 않습니다 — 그 원칙이 막는 것은 릴리즈 데드락을 부르는 **워크스페이스 체인**이지 서드파티가 아닙니다(`plan.md` 「제약」).
